...
\KwOut{The set of all orbit types for a cyclic permutation of length $n$ on $K_v^{(3)}$, as given by Definition \ref{def:orbit-types}}
...
\begin{algorithm}
\KwIn{$3$-uniform hypergraphs $H$ and $K$, where $K$ is isomorphic to one of $K_{v}^{(3)}$, $L_{v,v,[w]}^{(3)}$, $K_{v,v,v}^{(3)}$ for some $v, w$}
